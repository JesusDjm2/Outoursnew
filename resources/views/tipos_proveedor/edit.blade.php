@extends('layouts.app')
@section('title', 'Editar Tipo de Proveedor')
@section('content')
<h1 class="text-xl font-bold text-gray-800 mb-5 dark:text-slate-100">Editar Tipo de Proveedor</h1>
<div class="bg-white rounded-xl shadow p-5 md:p-6 max-w-md dark:bg-slate-900 dark:shadow-slate-950/50">
    <form method="POST" action="{{ route('tipos-proveedor.update', $tipo) }}" class="space-y-3">
        @csrf @method('PUT')
        <div>
            <label class="block text-xs font-medium text-gray-600 mb-1 dark:text-slate-400">Nombre</label>
            <input type="text" name="nombre" value="{{ old('nombre', $tipo->nombre) }}" required class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:placeholder-slate-500">
