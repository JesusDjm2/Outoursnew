...
@section('content')
<h1 class="text-xl font-bold text-gray-800 mb-5 dark:text-slate-100">Nuevo Hotel</h1>
<div class="bg-white rounded-xl shadow p-5 md:p-6 max-w-2xl dark:bg-slate-900 dark:shadow-slate-950/50">
    <form method="POST" action="{{ route('hotels.store') }}" enctype="multipart/form-data" class="space-y-3">
        @csrf
        @if($destinos->isEmpty())
        ...
        <div>
            ...
            <select name="destino_id" required class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100">
                ...
                @foreach($destinos as $destino)
                    <option value="{{ $destino->id }}" {{ (string) old('destino_id') === (string) $destino->id ? 'selected' : '' }}>{{ $destino->nombre }}</option>
                @endforeach
            </select>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div>
                ...
                <input type="text" name="codigo" value="{{ old('codigo') }}" list="hoteles-codigos-datalist" maxlength="20" class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:placeholder-slate-500 uppercase" placeholder="TE, TEA...">
            ...
            <div>
                ...
                <input type="text" name="direccion" value="{{ old('direccion') }}" class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:placeholder-slate-500">
            </div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
                <label class="block text-xs font-medium text-gray-600 mb-1 dark:text-slate-400">Teléfono</label>
                <input type="text" name="telefono" value="{{ old('telefono') }}" class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:placeholder-slate-500">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-600 mb-1 dark:text-slate-400">Email</label>
                <input type="email" name="email" value="{{ old('email') }}" class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:placeholder-slate-500">
            </div>
        </div>
        <div>
