@extends('layouts.app')
@section('title', 'Nuevo Paquete de Itinerarios')
@section('content')
@php $itemsSeleccionados = $itemsSeleccionados ?? collect(); @endphp
<h1 class="text-2xl font-bold text-gray-800 mb-6 dark:text-slate-100">Nuevo Paquete de Itinerarios</h1>
<div class="bg-white rounded-xl shadow p-6 md:p-8 dark:bg-slate-900 dark:shadow-slate-950/50">
    <form method="POST" action="{{ route('itinerary-packages.store') }}">
        ...
        <div class="mb-6">
            ...
            <textarea name="descripcion" rows="2" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100">{{ old('descripcion') }}</textarea>
